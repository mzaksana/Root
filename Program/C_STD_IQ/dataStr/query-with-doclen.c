...
{
	Term *terms;
	FILE *fvoc, *finv, *fpar, *finf, *fnme; //, *fzik;
	...
	if ((finv = fopen("Program/out/index-db/data.inv", "rb")) == NULL)
	{
		printf("Cannot open inverted file [data.inv]...\n");
		return 0;
	}

	if ((fpar = fopen("Program/out/index-db/data.par", "rb")) == NULL)
	{
		printf("Cannot open parameter file [data.par]...\n");
		return 0;
	}

	if ((fvoc = fopen("Program/out/index-db/data.voc", "r")) == NULL)
	{
		printf("Cannot open vocabulary file [data.voc]...\n");
		return 0;
	}
	if ((finf = fopen("Program/out/index-db/data.wdl", "r")) == NULL)
	{
		printf("Cannot open information file [data.wdl]...\n");
		return 0;
	}

	if ((fnme = fopen("Program/out/index-db/data.nme", "r")) == NULL)
	{
...
int searchTerm(Term *arrterm, int num, char *query)
{
	...
	while (index-- > 0)
	{
		if (strcmp(arrterm[index].term, query) == 0)
		{
			// printf("text : %s idx : %d\n", arrterm[index].term, index);
			return index;
		}
	}
	return -1;
